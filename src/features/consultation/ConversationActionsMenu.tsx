...
export function ConversationActions({
  ...
  const [renameInputElement, setRenameInputElement] = useState<HTMLInputElement | null>(null)
  ...
  const actions = (
    <>
      ...
      {isRenameOpen ? portal(
        <ConversationActionDialog title="重命名对话" onClose={() => setIsRenameOpen(false)}>
          <form
          ...
          >
            <label htmlFor={`conversation-rename-${consultation.id}`}>对话名称</label>
            <input
            ...
            <div className="conversation-action-dialog-actions">
              <button type="button" onClick={() => setIsRenameOpen(false)} disabled={isWorking}>
                取消
              </button>
      ...
      {isDeleteOpen ? portal(
        <ConversationActionDialog title="删除对话" onClose={() => setIsDeleteOpen(false)}>
          <div className="conversation-action-form">
            <p>删除后，这条对话将从对话记录中移除。</p>
            {errorMessage ? <p role="alert">{errorMessage}</p> : null}
            <div className="conversation-action-dialog-actions">
              <button type="button" onClick={() => setIsDeleteOpen(false)} disabled={isWorking}>
                取消
              </button>
              <button type="button" className="danger" onClick={() => void submitDelete()} disabled={isWorking}>
                {isWorking ? '删除中...' : '删除'}
              </button>
            </div>
          </div>
...
function ConversationActionDialog({
  ...
  return (
    <div className="conversation-action-dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section
