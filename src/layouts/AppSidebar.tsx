  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label="新对话"
        onClick={() => navigate(`/consultation/new?new=${Date.now()}`)}
      >
      ...
      <nav className="dashboard-nav">
        {sidebarNavigationItems.map((item) => (
          <NavLink
            key={item.to}
            className={
              isNavigationItemActive(item.to, item.match, location.pathname)
      ...
      >
        ...
        <nav className="sidebar-conversation-list" aria-label="最近对话">
          ...
          {conversationQuery.isPending ? (
            <span className="sidebar-conversation-empty">正在加载...</span>
          ) : null}
          {!conversationQuery.isPending && conversations.length === 0 ? (
            <span className="sidebar-conversation-empty">
              对话会自动保存在这里
            </span>
          ) : null}
        </nav>
          ...
          >
            ...
            >
              ...
              <span>
                ...
                {/* <small>{serviceSummary}</small> */}
              </span>
              <MaterialIcon name="chevronRight" />
            </button>
            <div className="account-menu-divider" />
            <div
              className="account-menu-versions"
              role="group"
              aria-label="版本信息"
            >
              <div className="account-menu-version-heading">
                <span>版本信息</span>
                <small>{systemVersionQuery.isFetching ? "正在更新" : ""}</small>
              </div>
              {versionEntries.map((entry) => (
                <div
                  className="account-menu-version-row"
                  key={entry.label}
                  title={readVersionTitle(
                    entry.startedAt,
                ...
                >
                  <span className="account-menu-version-service">
                    <i
                      className={`service-status-dot is-${entry.status}`}
                      aria-hidden="true"
                    />
                    {entry.label}
                  </span>
                  <span
                    className={`account-menu-version-value is-${entry.status}`}
                  >
            ...
            <button
              type="button"
              role="menuitem"
              onClick={() => navigateFromMenu("/knowledge")}
            >
...
function readVersionTitle(startedAt?: string, runtimeVersion?: string) {
  ...
  if (startedAt) {
    ...
    if (!Number.isNaN(date.getTime()))
      details.push(`服务启动于 ${date.toLocaleString("zh-CN")}`);
  }
